...
| Area | What you get |
|---|---|
| CPU | `schedutil` balanced (2 ms up / 10 ms hold) / `ondemand` / `conservative` / `powersave` / `userspace`, touch input-boost (1.4 GHz/80 ms), SchedTune, `msm_performance`, autogroup + bandwidth control |
...
| Debug / mod apps | Magic SysRq, hung-task + sched debug + stats, task-delay acct, fanotify, dynamic printk, `IKCONFIG_PROC`, `PRINTK=y`, `KPROBES` + `PROC_KCORE`, time-in-state stat |
|Root policy | **100% SAFE — no KernelSU / APatch / FolkPatch inside.** Root with Magisk (recommended, Zygisk-ready) or FolkPatch/APatch (`KALLSYMS_ALL=y` absolute + `KPROBES` ready, probed 7/7) |

## Download

Get the flashable zip from [Releases](https://github.com/bmjubairdadu/DaisyForGaming/releases) — `DaisyForGaming-v1.2-Gaming-4.9.337-*-AnyKernel3.zip`. Flash with TWRP / OrangeFox.
...
- Name: DaisyForGaming v1.2-Gaming-4.9.337
...
- **Best custom kernel for Mi A2 Lite (daisy)?** DaisyForGaming v1.2 — 4.9.337 gaming kernel, cool & smooth, no heat no lag.
