...
| Area | What you get |
|---|---|
| CPU | `schedutil` / `ondemand` / `conservative` / `powersave` / `userspace` governors, stock touch input-boost, SchedTune, `msm_performance` |
| GPU | `msm-adreno-tz` governor + `adreno_idler`, devfreq boost (stock 650 MHz table, no OC) |
| Network | TCP BBR + CUBIC available, `westwood` default (runtime: `echo bbr > /proc/sys/net/ipv4/tcp_congestion_control`) |
| Storage / IO | BFQ + CFQ (default) + NOOP, F2FS, ZRAM (`lz4`) |
| Display | KCAL color control, DT2W gesture sysfs (`fts_gesture_mode`) |
| Battery | Stock charging path; app-triggered bypass supported via existing `battery_charging_enabled` node (no patch needed) |
| Thermal | Video-smooth profile, throttle from 45 °C (stock-like) |
| System | Treble, Binder IPC, ION, OverlayFS, dm-verity, Fsync sysfs (`/sys/module/sync/parameters/fsync_enabled`) |
| Debugging | `IKCONFIG_PROC` (`/proc/config.gz`), `PRINTK=y` |
